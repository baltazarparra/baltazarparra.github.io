...
const liquidSlot = document.querySelector("[data-liquid-slot]");
...
const portraitUrl = new URL("./portrait.jpg", import.meta.url);
...
const init = async () => {
  if (!canvas || !smileSlot || !liquidSlot) return;
  ...
  const [geometryResponse, portrait] = await Promise.all([
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, portrait);
  ...
  const state = {
    ...
    mediaHoverTarget: 0,
    mediaVelocity: 0,
    mediaVelocityTarget: 0,
    lastMediaProcessedAt: 0,
    surfaceActive: false,
    running: false,
    visible: true,
    lastFrameAt: 0,
    ...
    liquidRect: liquidSlot.getBoundingClientRect(),
  };
  ...
  const drawMedia = (time) => {
    ...
    gl.uniform2f(mediaUniforms.uPointer, state.mediaX, state.mediaY);
    gl.uniform1f(mediaUniforms.uHover, state.mediaHover);
    gl.uniform1f(mediaUniforms.uVelocity, state.mediaVelocity);
    gl.uniform1f(mediaUniforms.uTime, time * 0.001);
    gl.uniform1f(mediaUniforms.uViewportAspect, viewport.width / viewport.height);
    gl.uniform1f(mediaUniforms.uTextureAspect, portrait.naturalWidth / portrait.naturalHeight);
    gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
  };
  ...
  const updateRects = () => {
    ...
    state.liquidRect = liquidSlot.getBoundingClientRect();
  };
  ...
  const render = (time) => {
    ...
    state.mediaHover += (state.mediaHoverTarget - state.mediaHover) * 0.11;
    state.mediaVelocity += (state.mediaVelocityTarget - state.mediaVelocity) * 0.14;
    state.mediaVelocityTarget *= 0.88;
  ...
  const updateGlobalPointer = (event) => {
    ...
    const liquidRect = state.liquidRect;
    const overLiquid =
      event.clientX >= liquidRect.left && event.clientX <= liquidRect.right &&
      event.clientY >= liquidRect.top && event.clientY <= liquidRect.bottom;
  ...
  const updateMediaPointer = (event) => {
    if (softwareRenderer) return;
    const now = performance.now();
    if (now - state.lastMediaProcessedAt < 28) return;
    state.lastMediaProcessedAt = now;
    const rect = state.liquidRect;
    const x = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
    const y = Math.max(0, Math.min(1, 1 - (event.clientY - rect.top) / rect.height));
    const distance = Math.hypot(x - state.mediaX, y - state.mediaY);
    state.mediaX = x;
    state.mediaY = y;
    state.mediaVelocityTarget = Math.min(1, distance * 7.5);
    state.mediaHoverTarget = 1;
    requestFrame(700);
  };

  const leaveMedia = () => {
    if (softwareRenderer) return;
    state.mediaHoverTarget = 0;
    state.mediaVelocityTarget = 0.45;
    requestFrame(700);
  };
  ...
  liquidSlot.addEventListener("pointerenter", updateMediaPointer, { passive: true });
  liquidSlot.addEventListener("pointermove", updateMediaPointer, { passive: true });
  liquidSlot.addEventListener("pointerleave", leaveMedia, { passive: true });
  liquidSlot.addEventListener("pointercancel", leaveMedia, { passive: true });
  ...
  if (new URLSearchParams(window.location.search).get("view") === "liquid") {
    const rect = liquidSlot.getBoundingClientRect();
    const target = window.scrollY + rect.top - (window.innerHeight - rect.height) * 0.5;
  ...
  document.documentElement.dataset.motion = reducedMotion.matches ? "reduced" : "full";
  if (status) {
    status.textContent = softwareRenderer
